...
def destination_review_list_view(request, destination_id):
    destination = get_object_or_404(Destination, pk=destination_id)
    reviews = DestinationReview.objects.filter(destination=destination).select_related('user').order_by('-created_at')
...
def accommodation_review_list_view(request, accommodation_id):
    accommodation = get_object_or_404(Accommodation, pk=accommodation_id)
    reviews = AccommodationReview.objects.filter(accommodation=accommodation).select_related('user').order_by('-created_at')
